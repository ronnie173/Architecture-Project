...
public class RetrieveMemberProcessor extends MessageProcessor implements
		MessageListener {
	private final static Logger logger = Logger
			.getLogger(RetrieveMemberProcessor.class);
	...
	public void onMessage(Message message) {
		...
		try {
			...
			if (member == null) {
				...
				logger.debug("Found Member: " + member.getFirstName() + " "
						+ member.getLastName());
				...
				logger.debug("Finished Sending: " + member.getFirstName() + " "
						+ member.getLastName());
			}
		} catch (Exception ex) {
			logError("RetrieveMemberProcessor.onMessage() " + ex.getMessage());
		}
	}
}
