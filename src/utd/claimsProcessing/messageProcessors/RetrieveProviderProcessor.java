...
/**
 * A message processor responsible for retrieving the Member identified by the
 * Claim from the MemberDAO. The retrieved member is attached to the ClaimFolder
 * before passing to the next step in the process.
 */

public class RetrieveProviderProcessor extends MessageProcessor implements
		MessageListener {
	private final static Logger logger = Logger
			.getLogger(RetrieveProviderProcessor.class);
	...
	public void onMessage(Message message) {
		...
		try {
			...
			Provider provider = ProviderDAO.getSingleton().retrieveProvider(
					providerID);
			if (provider == null) {
				...
				logger.debug("Provider Member: " + provider.getProviderName());
			...
			logError(
					"RetrieveProviderProcessor.onMessage() " + ex.getMessage(),
					ex);
		}
	}
}
